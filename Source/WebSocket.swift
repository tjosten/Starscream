...
public class WebSocket : NSObject, StreamDelegate {
    ...
    */
    ...
    private func initStreamsWithData(_ data: Data, _ port: Int) {
        ...
        let bytes = UnsafeRawPointer((data as NSData).bytes)
        ...
        writeQueue.addOperation { [weak self] in
            ...
            let unsafeBytes: UnsafePointer<UInt8> = bytes.assumingMemoryBound(to: UInt8.self)
            outStream.write(unsafeBytes, maxLength: data.count)
        }
    }
    //delegate for the stream methods. Processes incoming bytes
    public func stream(_ aStream: Stream, handle eventCode: Stream.Event) {
        if let sec = security, !certValidated && [.hasBytesAvailable, .hasSpaceAvailable].contains(eventCode) {
            let possibleTrust: Any? = aStream.property(forKey: Stream.PropertyKey(rawValue:kCFStreamPropertySSLPeerTrust as String))
            if let trust: Any = possibleTrust {
                let domain: Any? = aStream.property(forKey: Stream.PropertyKey(rawValue:kCFStreamSSLPeerName as String))
    ...
    private func processInputStream() {
        ...
        let rawBuffer = UnsafeRawPointer(buf!.bytes)
        let buffer: UnsafeMutablePointer<UInt8> = UnsafeMutablePointer.init(mutating:rawBuffer.assumingMemoryBound(to: UInt8.self))
    ...
    private func dequeueInput() {
        while !inputQueue.isEmpty {
            ...
            let rawBuffer = ((work as NSData).bytes)
            let buffer :UnsafePointer<UInt8> = rawBuffer.assumingMemoryBound(to: UInt8.self)
    ...
    private func dequeueWrite(_ data: Data, code: OpCode, writeCompletion: (() -> ())? = nil) {
        writeQueue.addOperation { [weak self] in
            ...
            let rawBuffer = UnsafeRawPointer((frame!.mutableBytes))
            let buffer: UnsafeMutablePointer<UInt8> = UnsafeMutablePointer.init(mutating: rawBuffer.assumingMemoryBound(to: UInt8.self))
            ...
            while true {
                ...
                let rawBuffer = UnsafeRawPointer((frame!.bytes+total))
                let writeBuffer: UnsafePointer<UInt8> = rawBuffer.assumingMemoryBound(to: UInt8.self)
